...
def extract(filename : str, round_number : str):
    ...
    with pdf.open(filename) as file:
        ...
        while i < len(lines):
            ...
            m = re.match(r'(\d+\))\s*([A-Z &]+)\s*(Multiple Choice | Short Answer)\s*(.*)', line)
            ...
            if m != None:
                ...
                # Get full question
                i += 1
                while (i < len(lines) and not re.match(r'ANSWER:', lines[i])):
                    qtext += ' ' + lines[i].strip()
                    i += 1

                # Get answer
                answer = re.match(r'(ANSWER:)\s*(.*)', lines[i])
...
def play():
    ...
    while incomplete:
        ...
        while (q['subject'] not in subject):
            incomplete.pop(n)
...
# questions = []
# for n in range(1, 16):
#     questions += extract('Sample6_ROUND' + str(n) + '.pdf', str(n))

# to_json(questions, 'Set-6.json')
